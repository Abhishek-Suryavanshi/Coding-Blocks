# 🏆 Coding Blocks Course Certificates

This repository contains the certificates I have earned from **Coding Blocks**, showcasing my learning and achievements in **MERN full-stack development**, **DSA in C++**, and **Competitive Programming in C++**.  

---

## 📂 Certificates

| Course Name | Certificate | Platform |
|------------|------------|---------|
| Full Stack Web Development (MERN) | ![View Certificate](https://github.com/Abhishek-Suryavanshi/Coding-Blocks/blob/main/Full-Stack-Web-Development-MERN/70ca74d5-047f-4154-bdb8-34fc7c59515d.pdf) | Coding Blocks |
| Data Structures & Algorithms (C++) | ![View Certificate](https://github.com/Abhishek-Suryavanshi/Coding-Blocks/blob/main/DSA-C%2B%2B/Abhishek-Suryavanshi.pdf) | Coding Blocks |
| Competitive Programming (C++) | ![View Certificate](https://github.com/Abhishek-Suryavanshi/Coding-Blocks/blob/main/CP-C%2B%2B/CP-Certificate.pdf) | Coding Blocks |

---

## 🎯 Skills Gained

- **Programming Languages:** C++ (DSA & Competitive Programming), JavaScript (MERN)  
- **Web Development (MERN):** HTML, CSS, Bootstrap, Node.js, Express.js, MongoDB, Handlebars.js, React.js  
- **Algorithms & Data Structures:** Arrays, Linked Lists, Trees, Graphs, Sorting, Searching  
- **Competitive Programming:** Problem-solving, Time & Space Optimization, Contest Strategy  
- **Backend & APIs:** REST, gRPC  

---

## 📫 Connect With Me

<p>
  LinkedIn: 
  <a href="https://www.linkedin.com/in/abhishek-suryavanshi-0915a4197/" target="_blank">
    <img src="https://upload.wikimedia.org/wikipedia/commons/c/ca/LinkedIn_logo_initials.png" alt="LinkedIn" width="20" style="vertical-align: middle;">
  </a>
</p>


## 📜 License

This repository is for internal documentation and record-keeping purposes. Use and distribution are restricted as per company guidelines.
